    ...
    async function initAuth() {
      try {
        ...
        if (sessionError) {
          ...
          if (mounted) {
            setError(new Error(`Failed to get session: ${sessionError.message}`));
            setLoading(false);
          }
          return;
        }
        ...
        if (signInError) {
          ...
          if (signInError.message?.includes("anonymous") || signInError.status === 400) {
          ...
          if (mounted) {
            setError(new Error(`Failed to sign in anonymously: ${signInError.message}`));
            setLoading(false);
          }
          return;
        }
